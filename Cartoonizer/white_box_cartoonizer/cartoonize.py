...
class WB_Cartoonize:
    ...
    def cartoonize_video(self, input_video_path, output_video_path):
        ...
        if not out.isOpened():
            cap.release()
            raise ValueError(f"Could not create output video file '{output_video_path}'. Check codec support.")
        ...
        # Verify the video file is created and playable
        if os.path.exists(output_video_path) and os.path.getsize(output_video_path) > 0:
            print(f"Cartoonized video saved successfully as '{output_video_path}'")
        else:
